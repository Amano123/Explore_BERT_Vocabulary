...
def download_wiki40b_corpus(target: str) -> Tuple[DatasetInfo, List[str]]:
    ...
    ds, ds_info = tfds.load(
        name='wiki40b/ja', split=TARGET_DATASETS, with_info=True)
...
def get_args():
    parser = argparse.ArgumentParser(
        description='Download and parse target dataset.')
    parser.add_argument(
        '-t', '--target', choices=['train', 'validation', 'test'], help='Target dataset.')
